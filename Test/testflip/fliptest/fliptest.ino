...
const char* GESTURES[] = {
  "flex",
  "flip"  // Added "flip" to the gestures array
};
...
void setup() {
  ...
  Serial.println("Services and characteristics set up");
...
void loop() {
  ...
  if (central) {
    ...
    while (central.connected()) {
      if (!takeoffDetected) {
        int gesture = recognizeGesture();
        if (gesture == 1) {  // 1 corresponds to "flex"
        ...
        int gesture = recognizeGesture();
        if (gesture == 2) {  // 2 corresponds to "flip"
          sensorCharacteristic.writeValue("flip");
          Serial.println("Flip gesture detected!");
        }
      }
    ...
    if (IMU.accelerationAvailable() && IMU.gyroscopeAvailable()) {
      ...
      if (samplesRead == numSamples) {
        ...
        for (int i = 0; i < NUM_GESTURES; i++) {
          float confidence = modelGetOutput(i) * 100;
          if (confidence > maxConfidence && confidence > 90) {
