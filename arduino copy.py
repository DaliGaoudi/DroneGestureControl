...
def handle_data(sender: int, data: bytearray):
    ...
    logger.debug(f"Average values - Roll: {avg_roll}, Pitch: {avg_pitch}, Yaw: {avg_yaw}")
    ...
    logger.debug(f"Mapped commands - Roll: {roll_cmd}, Pitch: {pitch_cmd}, Yaw: {yaw_cmd}")
    ...
    logger.debug(f"Final commands after deadzone - Roll: {roll_cmd}, Pitch: {pitch_cmd}, Yaw: {yaw_cmd}")

    # Send control commands to Tello
    try:
        tello.send_rc_control(roll_cmd * 5, pitch_cmd,0, yaw_cmd)
        #logger.info(f"Commands sent: Roll: {roll_cmd}, Pitch: {pitch_cmd}, Yaw: {yaw_cmd}, Vertical: {vertical}")
        last_command_time = time.time()
